...
{
	return ((index + 1) % QUEUE_SIZE); // funkce zajistuje kruhovost pole pomoci operace modulo
}
...
{
	return nextIndex(queue->freeIndex) == queue->firstIndex; // pokud je index ,nasledujici za indexem, na ktery ukazuje freeIndex, stejny jako index na ktery ukazuje FirstIndex, fronta je plna
}
...
void Queue_Front(const Queue *queue, char *dataPtr)
{
	...
	{
		*dataPtr = queue->array[queue->firstIndex]; // ukladame hodnotu pole na prvnim indexu
	}
}
...
void Queue_Remove(Queue *queue)
{
	...
	{
		queue->firstIndex = nextIndex(queue->firstIndex); // prepojujeme ukazatel firstIndexu na nasledujici index
	}
}
...
void Queue_Enqueue(Queue *queue, char data)
{
	...
	{
		queue->array[queue->freeIndex] = data;			// do indexu, na ktery ukazuje freeIndex vkladame data
		queue->freeIndex = nextIndex(queue->freeIndex); // posouvame ukazatel freeIndexu na nasledujici volny prvek
	}
}
